{-# options_ghc -Wno-unused-imports -Wno-missing-pattern-synonym-signatures #-}
...
data Tm
  ...
  | Let Name Ty Tm Tm
  | Meta MetaVar
  | InsertedMeta MetaVar (List BD)    -- optimized representation of inserted metas
...
pattern VVar  x = VRigid x Nil
pattern VMeta m = VFlex m Nil


length :: List a -> Int
length = go 0 where
  go acc Nil         = acc
  go acc (Snoc as _) = go (acc + 1) as

infixl 8 $$
($$) :: Closure -> Val -> Val
($$) (Closure env t) ~u = eval (Snoc env u) t

vVar :: Env -> Ix -> Val
vVar (Snoc _  a) 0 = a
vVar (Snoc as _) n = vVar as (n - 1)
...
vAppBDs :: Env -> Val -> List BD -> Val
vAppBDs env ~v bds = case (env, bds) of
  (Nil        , Nil             ) -> v
  (Snoc env t , Snoc bds Bound  ) -> vAppBDs env v bds `vApp` t
  (Snoc env t , Snoc bds Defined) -> vAppBDs env v bds
  _                               -> error "impossible"
...
eval env = \case
  ...
  Let x _ t u        -> eval (Snoc env (eval env t)) u
  U                  -> VU
  Meta m             -> vMeta m
...
quoteSp l t = \case
  Nil       -> t
  Snoc sp u -> App (quoteSp l t sp) (quote l u)
...
rename m pren v = go pren v where
  ...
  goSp pren t (Snoc sp u) = App <$> goSp pren t sp <*> go pren u
...
type Types = HM.HashMap Name (Lvl, VTy)

data Cxt = Cxt {
    env   :: Env           -- evaluation
  , lvl   :: Lvl           -- evaluation
  , types :: Types         -- raw name lookup
  , bds   :: List BD       -- fresh meta creation
...
infer cxt = \case
  ...
  RVar x -> case HM.lookup x (types cxt) of
...
cxtNames :: Cxt -> [Name]  -- not efficient, but it doesn't matter much in printing
cxtNames cxt =
    map fst
...
prettyTm prec = go prec where
  ...
  goBDS :: Int -> [Name] -> MetaVar -> List BD -> ShowS
  goBDS p ns m bds = case (ns, bds) of
    ([]   , Nil              ) -> (("?"++show m)++)
    (n:ns , Snoc bds Bound   ) -> par p appp $ goBDS appp ns m bds . (' ':) . (n++)
    (n:ns , Snoc bds Defined ) -> goBDS appp ns m bds
    _                          -> error "impossible"
