...
module UnificationBackup where
...
data Expansion = Expansion {
    expansionFreshVar :: Lvl
  , expansionSub      :: IM.IntMap Val }

makeFields ''Expansion

forceWithExpansion :: Expansion -> Val -> Val
forceWithExpansion exp t = case force t of
  VRigid x sp | Just v <- IM.lookup (coerce x) (exp^.sub) ->
    forceWithExpansion exp (vAppSp v sp)
  t -> t
...
psubstSp :: Dbg => Expansion -> PartialSub -> Tm -> Spine -> IO Tm
psubstSp exp psub t = \case
  SNil              -> pure t
  SApp sp u i       -> App <$> psubstSp exp psub t sp <*> psubst exp psub u <*> pure i
  SProj1 sp         -> Proj1 <$> psubstSp exp psub t sp
  SProj2 sp         -> Proj2 <$> psubstSp exp psub t sp
  SProjField sp x n -> ProjField <$> psubstSp exp psub t sp <*> pure x <*> pure n

psubst :: Dbg => Expansion -> PartialSub -> Val -> IO Tm
psubst exp psub t = case forceWithExpansion exp t of
  VFlex m' sp -> do
    case psub^.occ of
      Nothing ->
        psubstSp exp psub (Meta m') sp
      Just m -> case compareMetas m m' of
        EQ -> throwIO UnifyError
        LT -> do
          mty' <- case lookupMeta m' of Unsolved _ a -> pure a; _ -> impossible
          mty' <- psubst exp (PSub (Just m) 0 0 mempty) mty'
  ...
  VRigid (Lvl x) sp -> case IM.lookup x (psub^.sub) of
    ...
    Just x' -> psubstSp exp psub (quote (psub^.dom) x') sp

  VLam x i t  -> Lam x i <$> psubst exp (lift psub) (t $$ VVar (psub^.cod))
  VPi x i a b -> Pi x i <$> psubst exp psub a <*> psubst exp (lift psub) (b $$ VVar (psub^.cod))
  VSg x a b   -> Sg x <$> psubst exp psub a <*> psubst exp (lift psub) (b $$ VVar (psub^.cod))
  VPair t u   -> Pair <$> psubst exp psub t <*> psubst exp psub u
  VU          -> pure U

invertVal :: Dbg => Lvl -> Lvl -> Lvl -> Val -> Spine -> Expansion -> PartialSub -> IO (Expansion, PartialSub)
invertVal gammau gammas gammap t rhsSp psub = go gammap t rhsSp psub where
  ...
  go :: Dbg => Lvl -> Val -> Spine -> Expansion -> PartialSub -> IO (Expansion, PartialSub)
  go gammap t rhsSp exp psub = case force t of
    VRigid x sp -> do
      ...
      (exp, spInv) <- invertSp gammas gammap gammap (psub^.dom) sp `catch` \case
        NeedExpansion -> undefined
        e             -> throwIO e

      rhs <- psubstSp exp spInv (Var (Ix (spineLen sp))) rhsSp
      _
      -- traceShowM ("invert param", x, psub^.dom, lams sp rhs)
      -- pure $ psub & sub %~ IM.insert (coerce x) (eval (idEnv (psub^.dom)) (lams sp rhs))

    VPair t u -> do
      (exp, psub) <- go gammap t (SProj1 rhsSp) exp psub
      go gammap u (SProj2 rhsSp) exp psub
    VLam x i t ->
      go (gammap + 1) (t $$ VVar gammap) (SApp rhsSp (VVar gammap) i) exp psub
    _ ->
      throwIO UnifyError

invertSp :: Dbg => Lvl -> Lvl -> Lvl -> Lvl -> Spine -> IO (Expansion, PartialSub)
invertSp gammau gammas gammap delta sp = go sp where
  go :: Spine -> IO (Expansion, PartialSub)
  go SNil =
    pure (Expansion (-1) mempty, PSub Nothing delta gammas mempty)
  go (SApp sp t i) = do
    (exp, spInv) <- go sp
    invertVal gammau gammas gammap t SNil exp (spInv & dom +~ 1)
  go _ =
