...
-- | Insert fresh implicit applications and projections.
...
-- | Insert fresh implicit and projections to a neutral term.
insert :: Cxt -> IO (Tm, VTy) -> IO (Tm, VTy)
...
check cxt t a = case (t, force a) of
  ...
  (t, VEx x a b) -> do
    p1 <- freshMeta cxt a
    t <- check cxt t (b $$ eval (env cxt) p1)
    pure (Pair p1 t)
...
infer cxt = \case
  ...
  P.Ex x a b -> do
    a <- case a of Nothing -> freshMeta cxt VU
