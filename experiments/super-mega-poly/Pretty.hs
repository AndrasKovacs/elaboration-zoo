...
prettyTm prec = go prec where
  ...
  go p ns = \case
    ...
    Ex (fresh ns -> x) a b    -> par p pip $ ("∃ "++).(x++). (" : "++).go letp ns a.
                                 (". "++) . go letp (x:ns) b

    Proj1 t                   -> par p projp $ go projp ns t . (".1"++)
    Proj2 t                   -> par p projp $ go projp ns t . (".2"++)
    Wk t                      -> go p (tail ns) t
    Pair t u                  -> ('{':).go letp ns t.goPair u.('}':) where
                                   goPair (Pair t u) = (", "++).go letp ns t.goPair u
                                   goPair t          = (", "++).go letp ns t
    Postponed _ t             -> go p ns t
